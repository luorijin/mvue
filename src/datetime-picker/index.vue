<template>
    <mv-picker @confirm="onConfirm" :columns="columns" 
        ref="picker" :value="defaultVal"  :title="title" @change="onChange">
    </mv-picker>
</template>
...
export default {
    ...
    columns() {
      const results = this.ranges.map(({ type, range: rangeArr }) => {
        const values = times(rangeArr[1] - rangeArr[0] + 1, index => {
          ...
          return {text:this.formatter(type, value),value:this.formatter(type, value)};
        });
  ...
  created() {
    let arr = this.getShowColumn(this.innerValue);
    this.defaultVal = arr.map((val)=>{
